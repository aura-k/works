...
while(mktime() < $expireTime){ 
	...
	$str = "SELECT A.item_id FROM BBanana_items AS A, BBanana_autobids AS B
		...
		AND A.item_expired - unix_timestamp(now()) >= -5
		AND B.auto_banana > 0
...
function autoAction($sid){
	...
	$str = "SELECT A.item_id, A.item_expired, A.item_price, A.item_fname, A.item_lastbider, B.auto_banana, B.bider_id, B.bider_ipnum, B.is_mobile
			...
			AND A.item_expired - unix_timestamp(now()) >= -5
			AND B.auto_banana > 0
			ORDER BY A.item_expired, rand() ASC limit 0, 1";
	$sql = mysql_query($str) or die(mysql_error());
	$row = mysql_fetch_array($sql);

	if(intval($row['item_expired']) - mktime() <= 0 && $row['item_expired'] - mktime() >= -5 && $row['item_lastbider'] != $row['bider_id']){
...
//////////////////////  여기서부터 새값에 대한 수정, 입력 SQL이 시작된다. //////////////////////
		$str_bids = "SELECT * from `BBanana_view_bids` WHERE item_id='".$row['item_id']."' AND bider_id = '".$row['bider_id']."'";//bids테이블 과부하 방지하기위해 여기다 갱신함
		$sql_bids = @mysql_query($str_bids); 
		$row_bids = mysql_fetch_array($sql_bids);

		if(!$row_bids){
			$query = "INSERT INTO `BBanana_view_bids`(`item_id`, `bider_id`, `bid_count`) VALUES('".$row['item_id']."', '".$row['bider_id']."', 1);";
			$result = @mysql_query($query); 
		}else{
			$query = "UPDATE `BBanana_view_bids` SET bid_count=bid_count+1 WHERE item_id='".$row['item_id']."' AND bider_id='".$row['bider_id']."';";
			$result = @mysql_query($query); 
		}
