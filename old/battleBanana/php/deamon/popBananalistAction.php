...
function banana_pagenation($num){
	...
	while($row=mysql_fetch_array($sql)){
		if(preg_match('/B_/i', $row['item_id'])){//바나나 충전일때
			...
			echo "<tr bgcolor=\"#FFFFFF\" class=\"con\">";
			echo "<td  height=\"20\">".date('Y-m-d H:i', intval($row['action_date']))."</td>";
			echo "<td class=\"text_br\"><a href=\"sub.html?sid=".substr($row['item_id'],0,6)."\">".$row['comment']."</a> <img src='../img/pop/btn_sign_auto.gif' border='0' align='absmiddle'></td>";
			echo "<td class=\"text_grey\">-</td>";
			echo "<td class=\"text_grey\" title=\"현재 ".$row['banana_out']."개 사용, ".$row2['auto_banana']."개 오토배틀 대기중\" alt=\"현재 ".$row['banana_out']."개 사용, ".$row2['auto_banana']."개 오토배틀 대기중\" style=\"cursor:help\">".$row['banana_out']." / ".$row2['auto_banana']."</td>";
			echo "</tr>";
		}else{//일반 배틀일때
			echo "<tr bgcolor=\"#FFFFFF\" class=\"con\">";
			echo "<td  height=\"20\">".date('Y-m-d H:i', intval($row['action_date']))."</td>";
