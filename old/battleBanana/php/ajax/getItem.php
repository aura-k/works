<?	
	...
	if($page_type == "m"){//메인페이지에 뿌려줄 Json을 만드는 분기문
		...
		return;
			...
			$sql2=mysql_query("select COUNT(item_id) AS cnt from BBanana_items where item_expired - unix_timestamp(now()) <= -5 and item_lastbider = '".$_SESSION['ID']."'") or die(mysql_error()); 
			$row2=mysql_fetch_array($sql2);
			$page_num = intval(($row2['cnt']-1)/9 + 1);
			if($page == ""){
				echo "{\"page\":\"".$page_num."\"}";
			...
			$str = "SELECT * from BBanana_items where item_expired - unix_timestamp(now()) <= -5 and item_lastbider = '".$_SESSION['ID']."' order by item_id DESC limit ".($page*9).", 9";
			$sql=mysql_query($str) or die(mysql_error()); 
		...
		if($sid != ""){
			...
			if($time_gap < -5) $remain_time = '경매종료';
