...
class INILog 
{
  ...
  function StartLog($dir, $mid) 
	{
		...
		$this->WriteLog( INFO, "START ".PROGRAM." ".$this->type." (V".VERSION."-".BUILDDATE."-ScriptModule)" );
		return true;
	}
  function WriteLog($debug, $data) 
    ...
    if( is_array( $data ) )
    {
        ...
        {
            fwrite( $this->handle, $pfx . strtoupper($key) . ":" . $val . "\r\n");
        }
    }
    else
    {
  ...
  function CloseLog($msg)
  {
		...
		$this->WriteLog( INFO, "END ".$this->type." ".$msg );
    fclose( $this->handle );
  }
