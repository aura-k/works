...
include "../../php/connect.php"; //디비 정의 페이지 
  ...
  session_start();								//주의:파일 최상단에 위치시켜주세요!!
	if(!$_SESSION['ID'] || !$HTTP_SESSION_VARS['INI_PRICE']){
		echo '정상적인 방법으로 접근하시오!';
		return;
	}
	/**************************
	 * 1. 라이브러리 인클루드 *
  ...
  $inipay->SetField("id_customer",$id_customer);    //user_id
	...
	if(substr($HTTP_SESSION_VARS['MOID'],0,1) == "B"){
		//$ship_price = $banana_sum * (BANANA_PRICE + (BANANA_PRICE * 0.1));
		$ship_price = $banana_sum * BANANA_PRICE;
	}else{
		$sql = mysql_query("SELECT item_price, item_rrp FROM `BBanana_items` where item_id = '".$ship_sid."';") or die(mysql_error()); 
		$row = mysql_fetch_array($sql);

		if($ship_type == 'win'){
			$ship_price = $row['item_price'] + TEAKBAE;
		}else{
			$sql2=mysql_query("select COUNT('no') as cnt from BBanana_bids where item_id='".$ship_sid."' and bider_id='".$_SESSION['ID']."'") or die(mysql_error());
			$row2=mysql_fetch_array($sql2);

			$ship_price = $row['item_rrp'] - ( $row2['cnt'] * BANANA_PRICE ) + TEAKBAE;
		}
	}

	$OriginalPrice = $ship_price;
	$PostPrice = $HTTP_SESSION_VARS['INI_PRICE']; 
	//echo '원가격:'.$OriginalPrice.'| 현재가격:'.$PostPrice;
	if ( $OriginalPrice != $PostPrice ){

		echo '<script name=javascript> window.alert("가격변조 시도를 발견하였습니다!!결제를 취소합니다."); window.close(); </script>';
		return;
	}
	////////////////////////////////////////////////////////////////
	...
	if($inipay->GetResult('ResultCode') == "00" && $inipay->GetResult('PayMethod') == "VBank"){//결제성공및 무통장입금일경우
		...
		if($goodname){//중복된 주문값을 찾고 주문값이 없을때만 DB에 저장
			$sql = mysql_query("
				INSERT INTO `BBanana_ships`(
					`order_num`, `item_fname`, `item_id`, `user_id`, `charged_banana`, `address`, `phone_num1`, `phone_num2`, `phone_num3`, `comment`, `win_comment`, `item_price`, `ship_name`, `ship_created`, `ship_type`, `ship_status`, `pay_method`, `invoice_no`, `bankInfo`, `is_deposit`
				) VALUES(
					...
					'".iconv("EUC-KR", "UTF-8", $win_comment)."',
		...
		if($goodname){//중복된 주문값을 찾고 주문값이 없을때만 DB에 저장
			$sql = mysql_query("
				INSERT INTO `BBanana_ships`(
					`order_num`, `item_fname`, `item_id`, `user_id`, `charged_banana`, `address`, `phone_num1`, `phone_num2`, `phone_num3`, `comment`, `win_comment`, `item_price`, `ship_name`, `ship_created`, `ship_type`, `ship_status`, `pay_method`, `invoice_no`, `is_deposit`
				) VALUES(
					'".$inipay->GetResult('MOID')."',
					'".iconv("EUC-KR", "UTF-8", $goodname)."',
					'".$ship_sid."',
...
<?php           
	...
	if($inipay->GetResult('PayMethod') == "Card" || $inipay->GetResult('PayMethod') == "VCard" ){
		$cardbank_code = $inipay->GetResult('CARD_BankCode');
		switch($cardbank_code){
			case "02": $get_cardbank_code = "한국산업은행";
				break;
			case "03": $get_cardbank_code = "기업은행";
				break;
			case "04": $get_cardbank_code = "국민은행(주택)";
				break;
			case "05": $get_cardbank_code = "외환은행";
				break;
			case "07": $get_cardbank_code = "수협";
				break;
			case "11": $get_cardbank_code = "농협";
				break;
			case "12": $get_cardbank_code = "단위농협";
				break;
			case "16": $get_cardbank_code = "축협";
				break;
			case "20": $get_cardbank_code = "우리은행";
				break;
			case "21": $get_cardbank_code = "신한(조흥)";
				break;
			case "23": $get_cardbank_code = "제일은행";
				break;
			case "25": $get_cardbank_code = "하나은행";
				break;
			case "26": $get_cardbank_code = "신한은행";
				break;
			case "27": $get_cardbank_code = "씨티은행(한미)";
				break;
			case "31": $get_cardbank_code = "대구은행";
				break;
			case "32": $get_cardbank_code = "부산은행";
				break;
			case "34": $get_cardbank_code = "광주은행";
				break;
			case "35": $get_cardbank_code = "제주은행";
				break;
			case "37": $get_cardbank_code = "전북은행";
				break;
			case "38": $get_cardbank_code = "강원은행";
				break;
			case "39": $get_cardbank_code = "경남은행";
				break;
			case "41": $get_cardbank_code = "비씨카드";
				break;
			case "53": $get_cardbank_code = "시티은행";
				break;
			case "54": $get_cardbank_code = "홍콩상하이은행";
				break;
			case "71": $get_cardbank_code = "우체국";
				break;
			case "81": $get_cardbank_code = "하나은행";
				break;
			case "83": $get_cardbank_code = "평화은행";
				break;
			case "87": $get_cardbank_code = "신세계";
				break;
			case "88": $get_cardbank_code = "신한(조흥)";
				break;
		}

		$card_code = $inipay->GetResult('CARD_Code');
		switch($card_code){
			case "01": $get_card_code = "외환";
				break;
			case "03": $get_card_code = "롯데";
				break;
			case "04": $get_card_code = "현대";
				break;
			case "06": $get_card_code = "국민";
				break;
			case "11": $get_card_code = "BC";
				break;
			case "12": $get_card_code = "삼성";
				break;
			case "13": $get_card_code = "LG";
				break;
			case "14": $get_card_code = "신한";
				break;
			case "15": $get_card_code = "한미";
				break;
			case "21": $get_card_code = "해외비자";
				break;
			case "22": $get_card_code = "해외마스터";
				break;
			case "23": $get_card_code = "JCB";
				break;
			case "24": $get_card_code = "해외아멕스";
				break;
			case "25": $get_card_code = "해외다이너스";
				break;
		}
		echo "<tr> 
				<td>결제금액</td>
