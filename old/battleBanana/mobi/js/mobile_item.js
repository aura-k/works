		...
		try{
			...
			if(J_exp == '경매종료'){
				...
				if(temp1[J_sid] != temp2[J_sid] && temp2[J_sid]){
					$('.highlightable_'+J_sid).animate({"marginLeft": "+=15px"}, 100);
					$('.highlightable_'+J_sid).animate({"marginLeft": "-=15px"}, 100);
...
function subItemList(sid){
	...
	$.getJSON('../php/ajax/getItem.php?p_type=s&sid='+sid, function(json){
		try{			
			...
			$('.sub_item_price').text(json.data.pri+' 원');
			...
			if(sub_temp1.substr(0,6) == sub_temp2.substr(0,6) && sub_temp1 != sub_temp2 && sub_temp2 != null){
					...
					$('.sub_item_price').animate({marginRight: "0px"}, 200);
					if(sub_temp_eff2 == 'do'){
						highlightFor('sub_item_expired');
					}
			}else{}
		}catch(e){}
	})});
