function move_category(fo_obj){
	var validator = xe.getApp('validator')[0];
	if(!validator) return false;
	if(!fo_obj.elements['_filter']) jQuery(fo_obj).prepend('<input type="hidden" name="_filter" value="" />');
	fo_obj.elements['_filter'].value = 'move_category';
	validator.cast('ADD_CALLBACK', ['move_category', function(form){
		...
		responses = ['error','message','xml_file','source_category_srl'];
		if(!confirm('이동하시겠습니까?')) return false;
		exec_xml('document','procDocumentMoveCategory', params, completeMoveCategory, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'move_category']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['move_category', {
