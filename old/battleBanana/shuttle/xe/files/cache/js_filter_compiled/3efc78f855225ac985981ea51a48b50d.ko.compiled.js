function insert_extra_var(fo_obj){
	var validator = xe.getApp('validator')[0];
	if(!validator) return false;
	if(!fo_obj.elements['_filter']) jQuery(fo_obj).prepend('<input type="hidden" name="_filter" value="" />');
	fo_obj.elements['_filter'].value = 'insert_extra_var';
	validator.cast('ADD_CALLBACK', ['insert_extra_var', function(form){
		...
		exec_xml('document','procDocumentAdminInsertExtraVar', params, completeInsertExtraVar, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'insert_extra_var']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['insert_extra_var', {
		'name': {required:true},
		'module_srl': {required:true},
		'eid': {required:true,rule:'alpha_number'}
	}]);
	validator.cast('ADD_MESSAGE', ['name', 'name']);
	validator.cast('ADD_MESSAGE', ['module_srl', 'module_srl']);
	validator.cast('ADD_MESSAGE', ['eid', '확장변수 이름']);
