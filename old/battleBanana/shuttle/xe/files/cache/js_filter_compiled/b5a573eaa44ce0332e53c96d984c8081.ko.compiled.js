function update_lang_select(fo_obj){
	var validator = xe.getApp('validator')[0];
	if(!validator) return false;
	if(!fo_obj.elements['_filter']) jQuery(fo_obj).prepend('<input type="hidden" name="_filter" value="" />');
	fo_obj.elements['_filter'].value = 'update_lang_select';
	validator.cast('ADD_CALLBACK', ['update_lang_select', function(form){
		...
		exec_xml('install','procInstallAdminSaveLangSelected', params, completeMessage, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'update_lang_select']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['update_lang_select', {
