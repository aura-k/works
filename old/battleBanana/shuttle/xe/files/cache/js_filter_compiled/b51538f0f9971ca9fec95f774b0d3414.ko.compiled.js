function insert_lang(fo_obj){
	var validator = xe.getApp('validator')[0];
	if(!validator) return false;
	if(!fo_obj.elements['_filter']) jQuery(fo_obj).prepend('<input type="hidden" name="_filter" value="" />');
	fo_obj.elements['_filter'].value = 'insert_lang';
	validator.cast('ADD_CALLBACK', ['insert_lang', function(form){
		...
		responses = ['error','message','name'];
		if(!confirm('등록하시겠습니까?')) return false;
		exec_xml('module','procModuleAdminInsertLang', params, completeInsertLang, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'insert_lang']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['insert_lang', {
		'lang_code': {required:true,rule:'alpha_number'}
	}]);
	validator.cast('ADD_MESSAGE', ['lang_code', '언어 코드']);
