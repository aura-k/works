function update_env_config(fo_obj){
	var validator = xe.getApp('validator')[0];
	if(!validator) return false;
	if(!fo_obj.elements['_filter']) jQuery(fo_obj).prepend('<input type="hidden" name="_filter" value="" />');
	fo_obj.elements['_filter'].value = 'update_env_config';
	validator.cast('ADD_CALLBACK', ['update_env_config', function(form){
		...
		exec_xml('install','procInstallAdminSaveTimeZone', params, completeMessage, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'update_env_config']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['update_env_config', {
		'time_zone': {required:true}
	}]);
	validator.cast('ADD_MESSAGE', ['time_zone', 'time_zone']);
