function insert_shortcut(fo_obj){
	var validator = xe.getApp('validator')[0];
	if(!validator) return false;
	if(!fo_obj.elements['_filter']) jQuery(fo_obj).prepend('<input type="hidden" name="_filter" value="" />');
	fo_obj.elements['_filter'].value = 'insert_shortcut';
	validator.cast('ADD_CALLBACK', ['insert_shortcut', function(form){
		...
		exec_xml('admin','procAdminInsertShortCut', params, filterAlertMessage, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'insert_shortcut']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['insert_shortcut', {
		'module': {required:true}
	}]);
	validator.cast('ADD_MESSAGE', ['module', '모듈']);
