function reset_layout_code(fo_obj){
	var validator = xe.getApp('validator')[0];
	if(!validator) return false;
	if(!fo_obj.elements['_filter']) jQuery(fo_obj).prepend('<input type="hidden" name="_filter" value="" />');
	fo_obj.elements['_filter'].value = 'reset_layout_code';
	validator.cast('ADD_CALLBACK', ['reset_layout_code', function(form){
		...
		exec_xml('layout','procLayoutAdminCodeReset', params, filterAlertMessage, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'reset_layout_code']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['reset_layout_code', {
		'layout_srl': {required:true},
		'code': {required:true}
	}]);
	validator.cast('ADD_MESSAGE', ['layout_srl', 'layout_srl']);
	validator.cast('ADD_MESSAGE', ['code', 'code']);
