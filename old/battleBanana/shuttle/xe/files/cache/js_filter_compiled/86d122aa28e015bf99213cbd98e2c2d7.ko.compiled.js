function insert_grant(fo_obj){
	var validator = xe.getApp('validator')[0];
	if(!validator) return false;
	if(!fo_obj.elements['_filter']) jQuery(fo_obj).prepend('<input type="hidden" name="_filter" value="" />');
	fo_obj.elements['_filter'].value = 'insert_grant';
	validator.cast('ADD_CALLBACK', ['insert_grant', function(form){
		...
		exec_xml('module','procModuleAdminInsertGrant', params, completeInsertGrant, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'insert_grant']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['insert_grant', {
		'module_srl': {required:true,rule:'number'}
	}]);
