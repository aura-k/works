function insert_layout(fo_obj){
	var validator = xe.getApp('validator')[0];
	if(!validator) return false;
	if(!fo_obj.elements['_filter']) jQuery(fo_obj).prepend('<input type="hidden" name="_filter" value="" />');
	fo_obj.elements['_filter'].value = 'insert_layout';
	validator.cast('ADD_CALLBACK', ['insert_layout', function(form){
		...
		responses = ['error','message','layout_srl'];
		if(!confirm('등록하시겠습니까?')) return false;
		exec_xml('layout','procLayoutAdminInsert', params, completeInsertLayout, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'insert_layout']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['insert_layout', {
		'title': {required:true}
	}]);
	validator.cast('ADD_MESSAGE', ['title', '제목']);
	validator.cast('ADD_MESSAGE', ['layout', '레이아웃']);
	validator.cast('ADD_MESSAGE', ['layout_type', 'layout_type']);
