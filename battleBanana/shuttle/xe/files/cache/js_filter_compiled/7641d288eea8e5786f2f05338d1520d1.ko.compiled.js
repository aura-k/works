function manage_checked_document(fo_obj){
	var validator = xe.getApp('validator')[0];
	if(!validator) return false;
	if(!fo_obj.elements['_filter']) jQuery(fo_obj).prepend('<input type="hidden" name="_filter" value="" />');
	fo_obj.elements['_filter'].value = 'manage_checked_document';
	validator.cast('ADD_CALLBACK', ['manage_checked_document', function(form){
		...
		exec_xml('document','procDocumentManageCheckedDocument', params, completeManageDocument, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'manage_checked_document']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['manage_checked_document', {
