function update_layout_info(fo_obj){
	var validator = xe.getApp('validator')[0];
	if(!validator) return false;
	if(!fo_obj.elements['_filter']) jQuery(fo_obj).prepend('<input type="hidden" name="_filter" value="" />');
	fo_obj.elements['_filter'].value = 'update_layout_info';
	validator.cast('ADD_CALLBACK', ['update_layout_info', function(form){
		...
		exec_xml('layout','procLayoutAdminUpdate', params, filterAlertMessage, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'update_layout_info']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['update_layout_info', {
		'title': {required:true}
	}]);
