function delete_category(fo_obj){
	var validator = xe.getApp('validator')[0];
	if(!validator) return false;
	if(!fo_obj.elements['_filter']) jQuery(fo_obj).prepend('<input type="hidden" name="_filter" value="" />');
	fo_obj.elements['_filter'].value = 'delete_category';
	validator.cast('ADD_CALLBACK', ['delete_category', function(form){
		...
		responses = ['error','message','xml_file','category_srl'];
		if(!confirm('삭제하시겠습니까?')) return false;
		exec_xml('document','procDocumentDeleteCategory', params, completeDeleteCategory, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'delete_category']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['delete_category', {
