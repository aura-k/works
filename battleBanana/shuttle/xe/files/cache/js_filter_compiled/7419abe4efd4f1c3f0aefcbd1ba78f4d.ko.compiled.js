function install_ftp_path(fo_obj){
	var validator = xe.getApp('validator')[0];
	if(!validator) return false;
	if(!fo_obj.elements['_filter']) jQuery(fo_obj).prepend('<input type="hidden" name="_filter" value="" />');
	fo_obj.elements['_filter'].value = 'install_ftp_path';
	validator.cast('ADD_CALLBACK', ['install_ftp_path', function(form){
		...
		exec_xml('install','procInstallAdminSaveFTPPath', params, completeFtpPath, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'install_ftp_path']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['install_ftp_path', {
