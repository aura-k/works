function install_ftp_info(fo_obj){
	var validator = xe.getApp('validator')[0];
	if(!validator) return false;
	if(!fo_obj.elements['_filter']) jQuery(fo_obj).prepend('<input type="hidden" name="_filter" value="" />');
	fo_obj.elements['_filter'].value = 'install_ftp_info';
	validator.cast('ADD_CALLBACK', ['install_ftp_info', function(form){
		...
		exec_xml('install','procInstallAdminSaveFTPInfo', params, completeMessage, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'install_ftp_info']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['install_ftp_info', {
		'ftp_user': {required:true},
		'ftp_port': {required:true},
		'ftp_root_path': {required:true},
		'sftp': {}
	}]);
	validator.cast('ADD_MESSAGE', ['ftp_user', 'ftp_user']);
	validator.cast('ADD_MESSAGE', ['ftp_port', 'FTP port']);
	validator.cast('ADD_MESSAGE', ['ftp_root_path', 'ftp_root_path']);
	validator.cast('ADD_MESSAGE', ['sftp', 'SFTP 사용']);
