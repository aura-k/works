function update_layout_code(fo_obj){
	...
	validator.cast('ADD_CALLBACK', ['update_layout_code', function(form){
		...
		exec_xml('layout','procLayoutAdminCodeUpdate', params, filterAlertMessage, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'update_layout_code']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['update_layout_code', {
	...
	validator.cast('ADD_MESSAGE', ['code_css', 'code_css']);
