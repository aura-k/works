function insert_rss_module_config(fo_obj){
	var validator = xe.getApp('validator')[0];
	if(!validator) return false;
	if(!fo_obj.elements['_filter']) jQuery(fo_obj).prepend('<input type="hidden" name="_filter" value="" />');
	fo_obj.elements['_filter'].value = 'insert_rss_module_config';
	validator.cast('ADD_CALLBACK', ['insert_rss_module_config', function(form){
		...
		exec_xml('rss','procRssAdminInsertModuleConfig', params, filterAlertMessage, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'insert_rss_module_config']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['insert_rss_module_config', {
