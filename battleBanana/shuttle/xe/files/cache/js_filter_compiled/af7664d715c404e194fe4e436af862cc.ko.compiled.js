function insert_module_setup(fo_obj){
	var validator = xe.getApp('validator')[0];
	if(!validator) return false;
	if(!fo_obj.elements['_filter']) jQuery(fo_obj).prepend('<input type="hidden" name="_filter" value="" />');
	fo_obj.elements['_filter'].value = 'insert_module_setup';
	validator.cast('ADD_CALLBACK', ['insert_module_setup', function(form){
		...
		exec_xml('module','procModuleAdminModuleSetup', params, completeModuleSetup, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'insert_module_setup']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['insert_module_setup', {
		'module_srls': {required:true},
		'module_category_srl': {},
		'layout_srl': {},
		'skin': {required:true},
		'description': {},
	...
	validator.cast('ADD_MESSAGE', ['module_srls', 'module_srls']);
