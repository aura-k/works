function toggle_activate_addon(fo_obj){
	var validator = xe.getApp('validator')[0];
	if(!validator) return false;
	if(!fo_obj.elements['_filter']) jQuery(fo_obj).prepend('<input type="hidden" name="_filter" value="" />');
	fo_obj.elements['_filter'].value = 'toggle_activate_addon';
	validator.cast('ADD_CALLBACK', ['toggle_activate_addon', function(form){
		...
		exec_xml('addon','procAddonAdminToggleActivate', params, reloadDocument, responses, params, form);
	}]);
	validator.cast('VALIDATE', [fo_obj,'toggle_activate_addon']);
	return false;
};
...
(function($){
	var validator = xe.getApp('Validator')[0];
	if(!validator) return false;
	validator.cast('ADD_FILTER', ['toggle_activate_addon', {
		'addon': {required:true}
	}]);
	validator.cast('ADD_MESSAGE', ['addon', '애드온']);
